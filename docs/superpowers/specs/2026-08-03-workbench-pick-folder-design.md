...
- 刷新、关系图、导出 JSON
- ~~用正则设置页规则驱动扫描（后端/配置化扫描后续再接）~~ → 已由 `2026-08-04-workbench-regex-whitelist-scan-design.md` 承接；扫描不再硬编码约定根
- Dev 中间件自动扫描项目文件
...
| 扫描规则   | 启用白名单 `folderName`（根下第一层目录名）+ `filePattern`（文件名正则） |
| 点文件     | 路径 + 原文纯文本                                                |
| 工具栏     | 仅「选择文件夹」                                                 |
...
扫描规则详情见 `2026-08-04-workbench-regex-whitelist-scan-design.md`。
...
- ~~正则设置页规则驱动扫描~~（已实现，见 `2026-08-04-workbench-regex-whitelist-scan-design.md`）
- Markdown 渲染预览
- 刷新、关系分析、导出
...
| 日期       | 摘要                                                      |
| ---------- | --------------------------------------------------------- |
| 2026-08-03 | 实现选文件夹切片；状态改为已实现                          |
| 2026-08-04 | 子组件改为文件夹模块（Header / CatalogTree / RawPreview） |
| 2026-08-04 | 一度移除 scan 业务单测与 Vitest，后恢复 Vitest 基建       |
